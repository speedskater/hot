...
template<> __attribute__((always_inline)) inline uint64_t toBigEndianByteOrder<uint64_t>(uint64_t const & key) {
  return __bswap_64(key);
};

template<> inline uint32_t toBigEndianByteOrder<uint32_t>(uint32_t const & key) {
  return __bswap_32(key);
};

template<> inline uint16_t toBigEndianByteOrder<uint16_t>(uint16_t const & key) {
